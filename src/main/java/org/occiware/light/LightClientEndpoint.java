...
public class LightClientEndpoint {
    ...
    /**
     * Blocks until either the server sends a response to the request, an
     * communication error occurs or the communication request times out.
     *
     * @return the server response message.
     * @throws TimeoutException if the server does not respond before the
     * timeout value is reached.
     * @throws InterruptedException if the communication thread is interrupted
     * (e.g. thread.interrupt() is invoked on it for cancellation purposes).
